...
class Lambda {
    public:
        ...
        Lambda();
        Lambda(const matrix& A, const matrix& B, const vector<number>& pi);
};
...
class hmm {
    public:
        ...
        static Lambda mult_seq_estimate(const Lambda& lambda, const vector<matrix*>& alphas, const vector<matrix*>& betas, const vector<pair<vector<int>, int>>& observations);
